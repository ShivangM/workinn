...
const promiseServiceCategoryOptions = async (categoryId: string, subCategoryId: string, inputValue: string, callback: (res: ServiceCategory[]) => void) => {
    try {
        const res = await fetchServiceCategoriesOptions(categoryId, subCategoryId, inputValue);
        callback(res);
    } catch (error) {
...
const loadServiceCategoryOptions = async (categoryId: string, subCategoryId: string, inputValue: string) => {
    try {
        return await new Promise<ServiceCategory[]>((resolve) => {
            const debouncedPromise = debounce(async () => {
                await promiseServiceCategoryOptions(categoryId, subCategoryId, inputValue, (res) => {
                    resolve(res);
                });
            }, 300);
            debouncedPromise();
