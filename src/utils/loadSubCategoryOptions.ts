...
const promiseSubCategoryOptions = async (categoryId: string, inputValue: string, callback: (res: SubCategory[]) => void) => {
    try {
...
const loadSubCategoryOptions = async (categoryId: string, inputValue: string) => {
    try {
        return await new Promise<SubCategory[]>((resolve) => {
            const debouncedPromise = debounce(async () => {
                await promiseSubCategoryOptions(categoryId, inputValue, (res) => {
                    resolve(res);
                });
            }, 300);
            debouncedPromise();
