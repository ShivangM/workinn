...
const OwnerDetails = async ({ ownerId }: OwnerDetailsProps) => {
    ...
    return (
        <div className="flex space-x-4">
            <Image alt="" src={photoURL || "/assets/Dummy Profile.png"} height={40} width={40} className="object-cover rounded-full shadow" />
            <div className="flex flex-col space-y-1">
                <Link target='_blank' rel="noopener noreferrer" href={`/profile/${ownerId}`} className="text-sm font-semibold">{displayName}</Link>
